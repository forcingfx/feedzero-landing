...
The screenshot is the only artefact this skill produces. It no longer travels
with release notes: those moved to the app repo (`release-notes.mjs`), which
generates the Atom feed and serves it from `my.feedzero.app`. This repo
rewrites `/releases.xml` to that origin and must NOT contain a static
`releases.xml` — Vercel matches the filesystem before rewrites, so the file
would shadow the real feed. Run `node build.mjs` to refresh the homepage.
